...
# نستخدم مسار المؤقت /tmp لأن منصات Serverless مثل Vercel لا تسمح بالكتابة إلا في هذا المجلد
DB_NAME = "/tmp/keys_database.db"

def init_db():
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    # إنشاء الجدول المطور متضمناً كافة الحقول المطلوبة
    cursor.execute('''
...
# لوحة التحكم المتطورة مع ميزة النسخ التلقائي والأزرار المنسقة للموبايل
DASHBOARD_HTML = '''
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="container">
        ...
        <div class="subtitle">نظام التحكم بالمفاتيح بنقرة واحدة والفحص الصامت لـ Vercel</div>
...
def index():
    init_db()  # لضمان إنشاء قاعدة البيانات والجدول عند كل استدعاء على Vercel
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
...
# مهم لكي تتعرف منصة Vercel على التطبيق كموديول
